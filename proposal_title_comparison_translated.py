...
if not os.path.exists("results/translated-publics.txt"):
    with open("results/translated-publics.txt", "w", encoding="utf8") as translated:
        for sentence in sentences:
            ...
            sentences_translated.append(response_sentence)
            translated.write(response_sentence + "\n")
else:
    with open("results/translated-publics.txt", "r", encoding="utf8") as translated:
        sentences_translated = translated.readlines()

paraphrases = util.paraphrase_mining(model, sentences_translated)

with open('results/output_en_publics.log', 'w', encoding='utf8') as f:
    for paraphrase in paraphrases:
        score, i, j = paraphrase
        f.write("{} \t\t {} \t\t Score: {:.4f}\n".format(sentences_translated[i], sentences_translated[j], score))
